...
#if(VSECPRIM_ACTMD5_ENABLED == STD_ON)
...
VSECPRIM_ROM(VSECPRIM_LOCAL, actU32) sine[] =
{
...
VSECPRIM_FUNC(actRETURNCODE) actMD5Update(
   ...
   actMemCpyByteArray(buffer, &dataIN[lastBlockEnd], (info->buffer_used));
...
# define pad_len                                                      (actHASH_BLOCK_SIZE_MD5 - (2u * actMD5_WORD_SIZE))
   ...
   buffer[len] = 0x80u;
   len += 1u;
   actMemClear(&buffer[len], (actHASH_BLOCK_SIZE_MD5 - len));
   if(len > pad_len){
      actMD5Transform(buffer, digest, watchdog);
      actMemset(buffer, 0u, pad_len);
   }
   ...
   actMemClear(info->buffer, actHASH_BLOCK_SIZE_MD5);
