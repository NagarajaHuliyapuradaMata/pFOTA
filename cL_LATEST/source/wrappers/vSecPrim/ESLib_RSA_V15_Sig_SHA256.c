...
VSECPRIM_ROM(VSECPRIM_LOCAL, eslt_Byte) eslt_SIGN_RSA_SHA256_V15_ASN1_DIGESTINFO[] = ASN1_DIGESTINFO_SHA256;
...
VSECPRIM_FUNC(eslt_ErrorCode) esl_finalizeSignRSASHA256_V15(VSECPRIM_P2VAR_PARA(eslt_WorkSpaceRSAsig) workSpace
  ...
  if(workSpace == NULL_PTR){
    ...
    if(ESL_WST_ALGO_RSA != (workSpace->header.status & ESL_WST_M_ALGO)){
      ...
      messageBuf[i] = 0x00;
      i = i + 1u;
      actMemCpyByteArray((VSECPRIM_P2VAR_PARA(actU8)) (&messageBuf[i]), eslt_SIGN_RSA_SHA256_V15_ASN1_DIGESTINFO, sizeof(eslt_SIGN_RSA_SHA256_V15_ASN1_DIGESTINFO));
