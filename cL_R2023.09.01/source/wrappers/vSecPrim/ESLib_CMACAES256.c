...
VSECPRIM_FUNC(eslt_ErrorCode) esl_initCMACAES256(
  ...
  if((workSpace == NULL_PTR) || (key == NULL_PTR)){
    ...
    (void)actCMACAESInit(&workSpace->wsCMACAESbuffer, key, ESL_SIZEOF_AES256_KEY, workSpace->header.watchdog);

    workSpace->wsCMACAES = (VSECPRIM_P2VAR_PARA(eslt_Byte))&(workSpace->wsCMACAESbuffer);

    esl_SetWorkspaceStatus(&workSpace->header, ESL_WST_ALGO_CMACAES256);
  }
...
VSECPRIM_FUNC(eslt_ErrorCode) esl_updateCMACAES256(
  ...
  else if((retVal = esl_CheckWorkSpaceHeaderAndState(&workSpace->header, ESL_MAXSIZEOF_WS_CMACAES, ESL_WST_ALGO_CMACAES256)) != ESL_ERC_NO_ERROR){
  }
  else{
...
VSECPRIM_FUNC(eslt_ErrorCode) esl_finalizeCMACAES256(
  ...
  if((workSpace == NULL_PTR) || (messageCMAC == NULL_PTR)){
    ...
    *messageCMACLength = ESL_SIZEOF_AES256_BLOCK;
...
VSECPRIM_FUNC(eslt_ErrorCode) esl_verifyCMACAES256(
  ...
  if((workSpace == NULL_PTR) || (messageCMAC == NULL_PTR)){
    ...
    (void)esl_finalizeCMACAES256(workSpace, tmpMAC, &tmpMACLength);

    if(actMemcmp(tmpMAC, messageCMAC, ESL_SIZEOF_AES256_BLOCK) == FALSE){
