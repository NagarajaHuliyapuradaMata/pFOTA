...
#if(actAES_SPEED_UP <= VSECPRIM_SPEED_UP_LEVEL_1)
...
VSECPRIM_LOCAL_FUNC(void) actAESInitializePrevBlock(
  VSECPRIM_P2VAR_PARA(actAESSTRUCT) info
   ,  VSECPRIM_P2CONST_PARA(actU8) iv){
  if(iv == NULL_PTR){
    actMemset(info->prev_block, 0u, actAES_BLOCK_SIZE);
  }
  else{
    actMemCpyByteArray(info->prev_block, iv, actAES_BLOCK_SIZE);
  }
}
...
VSECPRIM_LOCAL_FUNC(void) actAESDecryptMultipleBlocksCBC(
  ...
  for(bcount = 0u; bcount < (blocks*actAES_BLOCK_SIZE); bcount += actAES_BLOCK_SIZE){
    actMemCpyByteArray(temp, &in[bcount], actAES_BLOCK_SIZE);
...
VSECPRIM_LOCAL_FUNC(void) actAESCTRIncrementCounter(VSECPRIM_P2VAR_PARA(actU8) counter){
  ...
  for(j = 1u; j <= actAES_BLOCK_SIZE; j++){
    ...
    counter[actAES_BLOCK_SIZE - j] = actGetLowByteFromU16(tmp);
  }

  counter[actAES_BLOCK_SIZE - 1u] += actGetU8fromU16(tmp, 2u);
}
...
#if(actAES_SPEED_UP == VSECPRIM_SPEED_UP_LEVEL_0)
  actU32 t;
  actU8 i, n;
  for(n = 0u; n < 4u; ++n){
    bo[n] = k[n];
...
# elif(actAES_SPEED_UP == VSECPRIM_SPEED_UP_LEVEL_1)
  actU32 t[4];
  actU8 n;

  for(n = 0u; n < 4u; ++n){
    t[0] = ACT_ROM_R_BYT(sbx_tab[actGetByte(bi[n], 0u)]);
    ...
    t[3] = ACT_ROM_R_BYT(sbx_tab[actGetByte(bi[(n + 3u) & 3u], 3u)]);
    t[3] = ((((t[3]) | (t[3] << 8u)) | (ff_mult_3(t[3]) << 16u)) | (ff_mult_2(t[3]) << 24u));
    bo[n] = t[0] ^ t[1] ^ t[2] ^ t[3] ^ k[n];
  }

# elif(actAES_SPEED_UP == VSECPRIM_SPEED_UP_LEVEL_2)
  actU8 n;

  for(n=0u; n < 4u; ++n){
    bo[n] = ft_tab[0][actGetByte(bi[n], 0u)] ^
            ft_tab[1][actGetByte(bi[(n+1u)&3u], 1u)] ^
            ft_tab[2][actGetByte(bi[(n+2u)&3u], 2u)] ^
            ft_tab[3][actGetByte(bi[(n+3u)&3u], 3u)] ^ k[n];
  }

# elif(actAES_SPEED_UP==3)
   bo[0] =  ft_tab[0][actGetByte(bi[0], 0u)] ^
            ft_tab[1][actGetByte(bi[1], 1u)] ^
            ft_tab[2][actGetByte(bi[2], 2u)] ^
            ft_tab[3][actGetByte(bi[3], 3u)] ^ k[0];
   bo[1] =  ft_tab[0][actGetByte(bi[1], 0u)] ^
            ft_tab[1][actGetByte(bi[2], 1u)] ^
            ft_tab[2][actGetByte(bi[3], 2u)] ^
            ft_tab[3][actGetByte(bi[0], 3u)] ^ k[1];
   bo[2] =  ft_tab[0][actGetByte(bi[2], 0u)] ^
            ft_tab[1][actGetByte(bi[3], 1u)] ^
            ft_tab[2][actGetByte(bi[0], 2u)] ^
            ft_tab[3][actGetByte(bi[1], 3u)] ^ k[2];
   bo[3] =  ft_tab[0][actGetByte(bi[3], 0u)] ^
            ft_tab[1][actGetByte(bi[0], 1u)] ^
            ft_tab[2][actGetByte(bi[1], 2u)] ^
            ft_tab[3][actGetByte(bi[2], 3u)] ^ k[3];
#else
...
# elif(actAES_SPEED_UP == VSECPRIM_SPEED_UP_LEVEL_1)
  ...
  for(n = 0u; n < 4u; ++n){
    t[0] = ACT_ROM_R_BYT(isb_tab[actGetByte(bi[n], 0u)]);
    ...
    t[3] = ACT_ROM_R_BYT(isb_tab[actGetByte(bi[(n + 1u) & 3u], 3u)]);
...
# elif(actAES_SPEED_UP == VSECPRIM_SPEED_UP_LEVEL_2)
  ...
  for(n = 0u; n < 4u; ++n){
    bo[n] = it_tab[0][actGetByte(bi[n], 0u)] ^
            it_tab[1][actGetByte(bi[(n+3u)&3u], 1u)] ^
            it_tab[2][actGetByte(bi[(n+2u)&3u], 2u)] ^
            it_tab[3][actGetByte(bi[(n+1u)&3u], 3u)] ^ k[n];
  }

# elif(actAES_SPEED_UP==3)
   bo[0] =  it_tab[0][actGetByte(bi[0], 0u)] ^
            it_tab[1][actGetByte(bi[3], 1u)] ^
            it_tab[2][actGetByte(bi[2], 2u)] ^
            it_tab[3][actGetByte(bi[1], 3u)] ^ k[0];
   bo[1] =  it_tab[0][actGetByte(bi[1], 0u)] ^
            it_tab[1][actGetByte(bi[0], 1u)] ^
            it_tab[2][actGetByte(bi[3], 2u)] ^
            it_tab[3][actGetByte(bi[2], 3u)] ^ k[1];
   bo[2] =  it_tab[0][actGetByte(bi[2], 0u)] ^
            it_tab[1][actGetByte(bi[1], 1u)] ^
            it_tab[2][actGetByte(bi[0], 2u)] ^
            it_tab[3][actGetByte(bi[3], 3u)] ^ k[2];
   bo[3] =  it_tab[0][actGetByte(bi[3], 0u)] ^
            it_tab[1][actGetByte(bi[2], 1u)] ^
            it_tab[2][actGetByte(bi[1], 2u)] ^
            it_tab[3][actGetByte(bi[0], 3u)] ^ k[3];
#else
#error actAES_SPEED_UP value not supported
#endif
}
...
# elif(actAES_SPEED_UP==3)
   ...
   bo[0] =           (actU32)sbx_tab[actGetByte(bi[0], 0u)] ^
            actRotl(((actU32)sbx_tab[actGetByte(bi[1], 1u)]),  8u) ^
            actRotl(((actU32)sbx_tab[actGetByte(bi[2], 2u)]), 16u) ^
            actRotl(((actU32)sbx_tab[actGetByte(bi[3], 3u)]), 24u) ^ k[0];
   bo[1] =           (actU32)sbx_tab[actGetByte(bi[1], 0u)] ^
            actRotl(((actU32)sbx_tab[actGetByte(bi[2], 1u)]),  8u) ^
            actRotl(((actU32)sbx_tab[actGetByte(bi[3], 2u)]), 16u) ^
            actRotl(((actU32)sbx_tab[actGetByte(bi[0], 3u)]), 24u) ^ k[1];
   bo[2] =           (actU32)sbx_tab[actGetByte(bi[2], 0u)] ^
            actRotl(((actU32)sbx_tab[actGetByte(bi[3], 1u)]),  8u) ^
            actRotl(((actU32)sbx_tab[actGetByte(bi[0], 2u)]), 16u) ^
            actRotl(((actU32)sbx_tab[actGetByte(bi[1], 3u)]), 24u) ^ k[2];
   bo[3] =           (actU32)sbx_tab[actGetByte(bi[3], 0u)] ^
            actRotl(((actU32)sbx_tab[actGetByte(bi[0], 1u)]),  8u) ^
            actRotl(((actU32)sbx_tab[actGetByte(bi[1], 2u)]), 16u) ^
            actRotl(((actU32)sbx_tab[actGetByte(bi[2], 3u)]), 24u) ^ k[3];
#else
#error actAES_SPEED_UP value not supported
#endif
}
...
# elif(actAES_SPEED_UP == 3)
   ...
   bo[0] =           (actU32)isb_tab[actGetByte(bi[0], 0u)] ^
            actRotl(((actU32)isb_tab[actGetByte(bi[3], 1u)]),  8u) ^
            actRotl(((actU32)isb_tab[actGetByte(bi[2], 2u)]), 16u) ^
            actRotl(((actU32)isb_tab[actGetByte(bi[1], 3u)]), 24u) ^ k[0];
   bo[1] =           (actU32)isb_tab[actGetByte(bi[1], 0u)] ^
            actRotl(((actU32)isb_tab[actGetByte(bi[0], 1u)]),  8u) ^
            actRotl(((actU32)isb_tab[actGetByte(bi[3], 2u)]), 16u) ^
            actRotl(((actU32)isb_tab[actGetByte(bi[2], 3u)]), 24u) ^ k[1];
   bo[2] =           (actU32)isb_tab[actGetByte(bi[2], 0u)] ^
            actRotl(((actU32)isb_tab[actGetByte(bi[1], 1u)]),  8u) ^
            actRotl(((actU32)isb_tab[actGetByte(bi[0], 2u)]), 16u) ^
            actRotl(((actU32)isb_tab[actGetByte(bi[3], 3u)]), 24u) ^ k[2];
   bo[3] =           (actU32)isb_tab[actGetByte(bi[3], 0u)] ^
            actRotl(((actU32)isb_tab[actGetByte(bi[2], 1u)]),  8u) ^
            actRotl(((actU32)isb_tab[actGetByte(bi[1], 2u)]), 16u) ^
            actRotl(((actU32)isb_tab[actGetByte(bi[0], 3u)]), 24u) ^ k[3];
#else
#error actAES_SPEED_UP value not supported
#endif
}

VSECPRIM_FUNC(actRETURNCODE) actAESKeySetup(VSECPRIM_P2VAR_PARA(actAESSTRUCT) info, VSECPRIM_P2CONST_PARA(actU8) key, actLengthType key_len, VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog) (void)){
  ...
  if(key == NULL_PTR){
    ...
    VSECPRIM_P2VAR_PARA(actU32) e_key = info->e_key;
    actU32 t;
    ...
    for(i = 0u; (i < iterations); ++i){
      ...
      t = actRotr(t, 8u);
      t = ls_box(t) ^ ACT_ROM_R_DWD(rco_tab[i]);
      for(j = 0u; j < key_dword_len; ++j)
      {
        if((j == 4u) && (key_dword_len == 8u))
        {
          t = e_key[((8u * i) + 4u) % 64u] ^ ls_box(t);
...
VSECPRIM_FUNC(void) actAESMakeDecryptionKey(VSECPRIM_P2VAR_PARA(actAESSTRUCT) info){
  ...
  for(i = 4u; i < iterations; ++i){
    ...
    w = star_x(v);
    t = w ^ tempKey[i];
    tempKey[i] = u ^ v ^ w ^ actRotr(u ^ t, 8) ^ actRotr(v ^ t, 16) ^ actRotr(t, 24);
  }
}
...
VSECPRIM_FUNC(actRETURNCODE) actAESInit(VSECPRIM_P2VAR_PARA(actAESSTRUCT) info, VSECPRIM_P2CONST_PARA(actU8) key, actLengthType key_len
   ,                                       VSECPRIM_P2CONST_PARA(actU8) iv, actU8 mode, VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog) (void)){
...
VSECPRIM_FUNC(void) actAESTransform(VSECPRIM_P2VAR_PARA(actAESSTRUCT) info
  ...
  if(transformBlock == TRUE){
...
#ifdef actNO_INDIRECT_CALLS
    if(transform == actAESEncryptMultipleBlocks){
      actAESEncryptMultipleBlocks(info, &in[diff], out_buffer, blocks, watchdog);
    }
    else{
      actAESDecryptMultipleBlocks(info, &in[diff], out_buffer, blocks, watchdog);
    }
#else
    ...
    actMemCpyByteArray(info->buffer, &in[diff + bytes], info->buffer_used);
  }
}
